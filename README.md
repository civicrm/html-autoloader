...
* In this example, we load the element-map from a JSON file (`elements.json`). It should be auto-generated by another process.
* Most resources (including the elment-map) benefit from browser caching.
* Some rules match an exact `element` name, and others rules match by `prefix`. We
...
* If a rule is activated, it will only load resources one time.
